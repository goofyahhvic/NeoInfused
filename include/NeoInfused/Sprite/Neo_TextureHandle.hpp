...
namespace neo {
    class TextureHandle {
        TextureHandle(id32_t id) : m_Id(id) {};
    public:
    ...
    public:
        ...
        inline void set_texture_b(const std::filesystem::path& path) { TextureRegistry::SetTextureB(m_Id, path); }
        // sets to a null texture if path is invalid
        inline void set_texture_n(const std::filesystem::path& path) { TextureRegistry::SetTextureN(m_Id, path); }
        ...
        inline void set_null_texture(const std::filesystem::path& path) { TextureRegistry::SetNullTexture(m_Id, path); }
        // won't destroy the original texture before setting the value, and will set to a blank texture if path is invalid.
        // useful for reusing memory, since destroyed textures stay in the texture registry so indecies don't move.
        inline void set_null_texture_b(const std::filesystem::path& path) { TextureRegistry::SetNullTextureB(m_Id, path); }
        // won't destroy the original texture before setting the value, and will set to a null texture if path is invalid.
        // useful for reusing memory, since destroyed textures stay in the texture registry so indecies don't move.
        inline void set_null_texture_n(const std::filesystem::path& path) { TextureRegistry::SetNullTextureN(m_Id, path); }
